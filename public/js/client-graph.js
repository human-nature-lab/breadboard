  ...
  vis.append('svg:defs').append('svg:marker')
    ...
    this.updateGraph = function (newGraph) {
        ...
        if (newGraph.nodes == undefined || newGraph.nodes.length == 0) {
            ...
            for(var i = nodes.length - 1; i >= 0; i--) {
                if (_.find(newGraph.nodes, function(n) {return n.id === nodes[i].id;}) === undefined) {
                    nodes.splice(i, 1);
                }
            }
            ...
            for(var i = 0; i < newGraph.nodes.length; i++) {
                var oldNode = _.find(nodes, function(n) {return n.id === newGraph.nodes[i].id;});
        ...
        if (newGraph.links == undefined || newGraph.links.length == 0) {
            ...
            for(var i = links.length - 1; i >= 0; i--) {
				...
				try {
					if (_.find(newGraph.links, function(l) { return ((newGraph.nodes[l.source].id === sourceId) && (newGraph.nodes[l.target].id === targetId));}) === undefined) {
						links.splice(i, 1);
					}
                } catch(e) {
            ...
            for(var i = 0; i < newGraph.links.length; i++) {
                ...
                var oldLink = _.find(links, function(l) { return ((l.target.id === targetId) && (l.source.id === sourceId)); });
	...
	var update = function () {
		...
		gEnter.append("svg:text")
			.attr("class", "node client")
			.style("text-anchor", "middle")
			.style("font-size", function(d) { return (d.id == clientId) ? "18pt" : "14pt" })
			.text(function(d) { return (d.score == undefined) ? "" : d.score; });

		gEnter.insert("svg:circle", "text.node")
			.attr("class", "node client")
			.attr("r", function(d) { return (d.id == clientId) ? egoNodeR : alterNodeR; } )
			.each(function(d) { if (d.id == clientId) { d.fixed = true; d.x = width / 2; d.y = height / 2; }} );
		...
		scoreText.text(function(d) { return (d.score == undefined) ? "" : d.score; });
